...
def download_additional_packages(SCR_PATH):
    commands = [
        f'curl -s -OL https://github.com/DEX-1101/sd-webui-notebook/raw/main/res/new_tunnel --output-dir {SCR_PATH}',
...
def monitor_aria2_download(header, args, dst_dir, out, url):
    """Запускает aria2c и перехватывает вывод для отображения прогресса загрузки."""
    try:
        command = f"aria2c {header} {args} -d {dst_dir} {out} '{url}'"

        # Запуск aria2c с перехватом вывода
...
def _handle_manual_download(url):
    """Обрабатывает загрузку для URL с префиксами."""
...
def _run_aria2c(prefix, url, dst_dir, file_name=None,args="", header=""):
    """Запускает загрузку с помощью aria2c."""
